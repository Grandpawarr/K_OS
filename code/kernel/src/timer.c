...
uint32_t msecs_to_jiffies(uint32_t ms) { return (ms * 1000 / SYS_TICK_US); }

/** @brief Convert microseconds to jiffies. @see timer.h */
uint32_t usecs_to_jiffies(uint32_t us) { return (us / SYS_TICK_US); }

/** @brief Convert jiffies to milliseconds. @see timer.h */
uint32_t jiffies_to_msecs(uint32_t js) { return (js * SYS_TICK_US / 1000); }

/** @brief Convert jiffies to microseconds. @see timer.h */
uint32_t jiffies_to_usecs(uint32_t js) { return (js * SYS_TICK_US); }
